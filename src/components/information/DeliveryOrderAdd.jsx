...
const DeliveryOrderAdd = ({ onCancel }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    deliveryDate: "",
    orderStatus: "NEW", // always NEW on creation, not editable
  });
  ...
  const handleProductSelected = async ({ product }) => {
    ...
    // Immediately set product fields; show loading state for price/stock
    setItems((prev) =>
      prev.map((item) =>
        item.tempId === tempId
          ? {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    request("POST", "/api/deliveryOrders", deliveryOrderData)
      .then(() => {
        onCancel(true);
      })
      .catch((error) => {
        ...
        setErrorMsg(
          error.response?.data?.message ||
            t(
              "deliveryOrderList.errors.addFailed",
              "Failed to add delivery order",
            ),
        );
        setLoading(false);
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
      <HeaderBar
        title={t("deliveryOrderList.addTitle", "Add Delivery Order")}
        showBackButton
