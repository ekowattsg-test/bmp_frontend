...
const UOMHierarchyAdd = ({ onCancel }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    request("POST", "/api/producthierarchies", {
        ...
        <Box sx={{ mb: 2 }}>
          <Autocomplete
            options={availableProducts.filter(
              (p) => !child || p.productId !== child.productId,
            )}
            ...
            renderInput={(params) => (
              <TextField
                ...
                placeholder={t("uomHierarchy.productPlaceholder")}
                required
              />
            )}
          />
...
export default UOMHierarchyAdd;
