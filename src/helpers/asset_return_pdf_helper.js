/**
 * Asset Return PDF Helper
 *
 * Generates an Asset Return Note PDF after an AssetReturn work order and stores
 * it in the document library.
 */
...
const ASSET_RETURN_LIBRARY_PROJECT_CODE = "__ASSET_RETURN";
const ASSET_RETURN_LIBRARY_CATALOG_NAME = "Asset Returns";
...
const buildQuickSearchKey = (workOrderId, from, to, fileName) => {
  return [workOrderId, from, to, fileName]
    .map((value) => String(value || "").trim())
    .filter(Boolean)
    .join(", ");
};

const ensureAssetReturnCatalog = async () => {
  try {
    const response = await request(
      "GET",
      `/api/librarycatelogs/project/${encodeURIComponent(ASSET_RETURN_LIBRARY_PROJECT_CODE)}`,
    );
  ...
  const createPayload = {
    libraryCatelogName: ASSET_RETURN_LIBRARY_CATALOG_NAME,
    active: 1,
    visibleLevel: 0,
    description: "Generated asset return documents",
    projectCode: ASSET_RETURN_LIBRARY_PROJECT_CODE,
    quicSearchKey: "asset return, return, asset",
  };
...
const renderAssetReturnPdf = async ({
  company,
  workOrderId,
  fromLocation,
  toLocation,
  ...
  pdf.setFontSize(16);
  pdf.text("Asset Return Note", pageWidth - margin, currentY, {
    align: "right",
  });
  currentY += 8;
  ...
  addLabelValue("From Worker", fromLocation);
  addLabelValue("To Location", toLocation);
  addLabelValue("Returned", formatDateTimeForPdf(new Date()));
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Returned Assets", margin, currentY);
  currentY += 5;
  ...
  if (validPhotos.length > 0) {
    ...
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(12);
    pdf.text("Return Photos", margin, currentY);
    currentY += 8;
...
/**
 * Generate an asset return PDF and store it in the document library.
 */
export const generateAndStoreAssetReturnPdf = async ({
  companyId,
  workOrderId,
  fromLocation,
  toLocation,
  ...
  if (!companyId) {
    throw new Error("companyId is required to generate the asset return PDF.");
  }
  if (!workOrderId) {
    throw new Error(
      "workOrderId is required to generate the asset return PDF.",
    );
  }
  ...
  if (!company) {
    throw new Error("Unable to resolve company details for asset return PDF.");
  }

  const fileName = buildFileName(workOrderId);
  const pdfBlob = await renderAssetReturnPdf({
    company,
    workOrderId,
    fromLocation,
    toLocation,
  ...
  try {
    const catalog = await ensureAssetReturnCatalog();
    const catalogId = Number(catalog?.libraryCatelogId);
    if (!Number.isFinite(catalogId) || catalogId <= 0) {
      throw new Error("Asset return document catalog is not available.");
    }
