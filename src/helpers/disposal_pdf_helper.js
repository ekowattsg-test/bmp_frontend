/**
 * Stock Disposal PDF Helper
 *
 * Generates a Stock Disposal Note PDF after a StockDisposal work order and
...
const DISPOSAL_LIBRARY_PROJECT_CODE = "__STOCK_DISPOSAL";
const DISPOSAL_LIBRARY_CATALOG_NAME = "Stock Disposals";
...
const buildFileName = (disposalId, workOrderId) => {
  const disposalPart = sanitizeFileNamePart(disposalId);
  const woPart = sanitizeFileNamePart(workOrderId);
  const date = sanitizeFileNamePart(formatDateForFileName(new Date()));
  return `${disposalPart}_${woPart}_${date}.pdf`;
};

const buildQuickSearchKey = (disposalId, workOrderId, location, fileName) => {
  return [disposalId, workOrderId, location, fileName]
    .map((value) => String(value || "").trim())
    .filter(Boolean)
    .join(", ");
};

const ensureDisposalCatalog = async () => {
  try {
    const response = await request(
      "GET",
      `/api/librarycatelogs/project/${encodeURIComponent(DISPOSAL_LIBRARY_PROJECT_CODE)}`,
    );
  ...
  const createPayload = {
    libraryCatelogName: DISPOSAL_LIBRARY_CATALOG_NAME,
    active: 1,
    visibleLevel: 0,
    description: "Generated stock disposal documents",
    projectCode: DISPOSAL_LIBRARY_PROJECT_CODE,
    quicSearchKey: "stock disposal, disposal, write off",
  };
...
const renderDisposalPdf = async ({
  ...
  pdf.setFontSize(16);
  pdf.text("Stock Disposal Note", pageWidth - margin, currentY, {
    align: "right",
  });
  currentY += 8;
  ...
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(10);
  addLabelValue("Disposal ID", disposalId);
  addLabelValue("Work Order", workOrderId);
  addLabelValue("Location", location);
  addLabelValue("Disposed By", disposedBy);
  addLabelValue("Reason", disposalReason);
  addLabelValue("Method", disposalMethod);
  addLabelValue("Disposed", formatDateTimeForPdf(new Date()));
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Disposed Items", margin, currentY);
  currentY += 5;
  ...
  if (validPhotos.length > 0) {
    ...
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(12);
    pdf.text("Disposal Photos", margin, currentY);
    currentY += 8;
...
/**
 * Generate a stock disposal PDF and store it in the document library.
 */
export const generateAndStoreDisposalPdf = async ({
  companyId,
  disposalId,
  workOrderId,
  location,
  ...
  if (!companyId) {
    throw new Error("companyId is required to generate the disposal PDF.");
  }
  if (!disposalId) {
    throw new Error("disposalId is required to generate the disposal PDF.");
  }
  if (!workOrderId) {
    throw new Error("workOrderId is required to generate the disposal PDF.");
  }
  ...
  if (!company) {
    throw new Error("Unable to resolve company details for disposal PDF.");
  }

  const fileName = buildFileName(disposalId, workOrderId);
  const pdfBlob = await renderDisposalPdf({
    company,
    disposalId,
    workOrderId,
    location,
  ...
  try {
    const catalog = await ensureDisposalCatalog();
    const catalogId = Number(catalog?.libraryCatelogId);
    if (!Number.isFinite(catalogId) || catalogId <= 0) {
      throw new Error("Stock disposal document catalog is not available.");
    }
