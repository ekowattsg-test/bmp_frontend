 ...
 * Return PDF Helper
 *
 * Generates a Stock Return Note PDF after a Return work order and stores it
...
const RETURN_LIBRARY_PROJECT_CODE = "__STOCK_RETURN";
const RETURN_LIBRARY_CATALOG_NAME = "Stock Returns";
...
const ensureReturnCatalog = async () => {
  try {
    const response = await request(
      "GET",
      `/api/librarycatelogs/project/${encodeURIComponent(RETURN_LIBRARY_PROJECT_CODE)}`,
    );
  ...
  const createPayload = {
    libraryCatelogName: RETURN_LIBRARY_CATALOG_NAME,
    active: 1,
    visibleLevel: 0,
    description: "Generated stock return documents",
    projectCode: RETURN_LIBRARY_PROJECT_CODE,
    quicSearchKey: "stock return, return, stock in",
  };
...
const renderReturnPdf = async ({
  ...
  pdf.setFontSize(16);
  pdf.text("Stock Return Note", pageWidth - margin, currentY, {
    align: "right",
  });
  currentY += 8;
  ...
  addLabelValue("From", fromLocation);
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Returned Items", margin, currentY);
  currentY += 5;
...
/**
 * Generate a stock return PDF and store it in the document library.
 */
export const generateAndStoreReturnPdf = async ({
  companyId,
  workOrderId,
  fromLocation,
  toLocation,
  ...
  if (!companyId) {
    throw new Error("companyId is required to generate the return PDF.");
  }
  if (!workOrderId) {
    throw new Error("workOrderId is required to generate the return PDF.");
  }
  ...
  if (!company) {
    throw new Error("Unable to resolve company details for return PDF.");
  }

  const fileName = buildFileName(workOrderId);
  const pdfBlob = await renderReturnPdf({
    company,
    workOrderId,
    fromLocation,
    toLocation,
  ...
  try {
    const catalog = await ensureReturnCatalog();
    const catalogId = Number(catalog?.libraryCatelogId);
    if (!Number.isFinite(catalogId) || catalogId <= 0) {
      throw new Error("Stock return document catalog is not available.");
    }
