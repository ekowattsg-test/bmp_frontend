 ...
 * Issue PDF Helper
 *
 * Generates a Stock Issue Note PDF after a Drawing work order and stores it
...
const ISSUE_LIBRARY_PROJECT_CODE = "__STOCK_ISSUE";
const ISSUE_LIBRARY_CATALOG_NAME = "Stock Issues";
...
const ensureIssueCatalog = async () => {
  try {
    const response = await request(
      "GET",
      `/api/librarycatelogs/project/${encodeURIComponent(ISSUE_LIBRARY_PROJECT_CODE)}`,
    );
  ...
  const createPayload = {
    libraryCatelogName: ISSUE_LIBRARY_CATALOG_NAME,
    active: 1,
    visibleLevel: 0,
    description: "Generated stock issue documents",
    projectCode: ISSUE_LIBRARY_PROJECT_CODE,
    quicSearchKey: "stock issue, drawing, stock out",
  };
...
const renderIssuePdf = async ({
  ...
  pdf.setFontSize(16);
  pdf.text("Stock Issue Note", pageWidth - margin, currentY, {
    align: "right",
  });
  currentY += 8;
  ...
  addLabelValue("Issued To", toLocation);
  addLabelValue("Issued", formatDateTimeForPdf(new Date()));
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Issued Items", margin, currentY);
  currentY += 5;
  ...
  if (validPhotos.length > 0) {
    ...
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(12);
    pdf.text("Issue Photos", margin, currentY);
    currentY += 8;
...
/**
 * Generate a stock issue PDF and store it in the document library.
 */
export const generateAndStoreIssuePdf = async ({
  companyId,
  workOrderId,
  fromLocation,
  toLocation,
  ...
  if (!companyId) {
    throw new Error("companyId is required to generate the issue PDF.");
  }
  if (!workOrderId) {
    throw new Error("workOrderId is required to generate the issue PDF.");
  }
  ...
  if (!company) {
    throw new Error("Unable to resolve company details for issue PDF.");
  }

  const fileName = buildFileName(workOrderId);
  const pdfBlob = await renderIssuePdf({
    company,
    workOrderId,
    fromLocation,
    toLocation,
  ...
  try {
    const catalog = await ensureIssueCatalog();
    const catalogId = Number(catalog?.libraryCatelogId);
    if (!Number.isFinite(catalogId) || catalogId <= 0) {
      throw new Error("Stock issue document catalog is not available.");
    }
